<?php
	...
	# preparing log file
	$logfile = $logdir . "validate_brand_" . $strdate2 . "_" . $struid . ".log";
	$invalidfile = $logdir . "invalid_brand_" . $strdate2 . "_" . $struid . ".log";
		...
		**/
		...
		if ($sheetCnt >= 1) {
			...
			for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {						
				...
				$brandName = $data->sheets[0]['cells'][$i][3];
				$description = $data->sheets[0]['cells'][$i][4];
				
				# entry test
				$check = $division . $code . $brandName . $description;
				
				if ($check != "") {
					
					if (empty($code)) {
						$invalidFound++;
						fwrite($h2, "Empty code found at row " . $i . "." . ENTER);
					}
					...
					# check brand
					$sql = "select count(id) CNT from mst_brand where upper(concat(name, division)) = upper(concat('" . mysql_real_escape_string($brandName) . "', '" . mysql_real_escape_string($division) . "'))";
					...
					if ($found) {
						$invalidFound++;
						fwrite($h2, "Brand at division exists found at row " . $i . "." . ENTER);
					}

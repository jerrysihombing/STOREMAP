<?php	
	...
	# preparing log file
	$logfile = $logdir . "process_brand_" . $strdate2 . "_" . $struid . ".log";
		...
		**/
		...
		if ($sheetCnt >= 1) {
			...
			for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {						
				...
				$code = mysql_real_escape_string($data->sheets[0]['cells'][$i][2]);
				$brandName = mysql_real_escape_string($data->sheets[0]['cells'][$i][3]);
				...
				if ($check != "") {
					...
					# -- INSERT ARTICLES --
					$sql = "insert into mst_brand (code, name, division, description, created_by, created_date) values ('" . $code . "', '" . $brandName . "', '" . $division . "', '" . $description . "', '" . $struid . "', '" . $strdate . "')";

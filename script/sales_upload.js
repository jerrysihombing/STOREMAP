            ...
            */
            ...
            $("#btnUpload").click(function() {
                        if (!ajaxInProgress) {
                            ajaxInProgress = true;
                            $("#trans_file_2").val("");
                            $("#salesUploaded").css("visibility", "hidden");
...
function dwForm() {
            location.href="/sales/dwform-sales.html";
}
			...
			success: function(data, status) {
				if (typeof(data.error) != "undefined") {
					if (data.error != "") {
						...
						uploadSuccess();
						$("#salesUploaded").css("visibility", "visible");
...
function goValidate() {
            ...
            if (fileToExe == "") {
                ...
                $.ajax({
                        type: "POST",
                        url: "../php/exe/sales_upload_validation.php",
                        data: dataString,
                        beforeSend: function() {
                            ajaxInProgress = true;
                            showProcessing();
...
function goSubmit() {
            ...
            $.ajax({
                    type: "POST",
                    url: "../php/exe/sales_upload_submit.php",
                    data: dataString,
                    beforeSend: function() {
                        ajaxInProgress = true;
                        showProcessing();
...
function goLogPreview(id) {
    ...
    //alert(newId); return;
    var url = "/sales/dwlog/"+newId+".html";

            ...
            */
            ...
            $("#btnUpload").click(function() {
                        if (!ajaxInProgress) {
                            ajaxInProgress = true;
                            $("#trans_file_2").val("");
                            $("#articleUploaded").css("visibility", "hidden");
...
function dwForm() {
            location.href="/article/dwform.html";
}
...
function goUpload() {
	...
	if (transfile == "") {
		...
		$.ajaxFileUpload({
			url: "../php/exe/article_upload.php",
			secureuri: false,
			...
			success: function(data, status) {
				if (typeof(data.error) != "undefined") {
					if (data.error != "") {
						...
						uploadSuccess();
						$("#articleUploaded").css("visibility", "visible");
...
function goValidate() {
            ...
            if (fileToExe == "") {
                ...
                $.ajax({
                        type: "POST",
                        url: "../php/exe/article_upload_validation.php",
                        data: dataString,
                        beforeSend: function() {
                            ajaxInProgress = true;
                            showProcessing();
...
function goSubmit() {
            ...
            $.ajax({
                    type: "POST",
                    url: "../php/exe/article_upload_submit.php",
                    data: dataString,
                    beforeSend: function() {
                        ajaxInProgress = true;
                        showProcessing();
...
function goLogPreview(id) {
    ...
    //alert(newId); return;
    var url = "/article/dwlog/"+newId+".html";
